...
export function setupTokenizer(language: MonacoEditorLanguage) {
	...
	languages.onLanguageEncountered(language, () => {
		languages.registerColorProvider(language, {
			provideColorPresentations(model, info) {
				...
				const rgb = [
					...
					Math.round(info.color.green * 255),
					Math.round(info.color.blue * 255),
				];
				return [
					{
						label: `rgb(${rgb.join(", ")})`,
