...
export class TypeScriptWorker implements ts.LanguageServiceHost, ITypeScriptWorker {
	...
	public async getSyntacticDiagnostics(fileName: string): Promise<Diagnostic[]> {
	...
	@TypeScriptWorker.clearFiles
	public async getSemanticDiagnostics(fileName: string): Promise<Diagnostic[]> {
		if (fileNameIsLib(fileName)) {
			return [];
		}
	...
	@TypeScriptWorker.clearFiles
	public async getSuggestionDiagnostics(fileName: string): Promise<Diagnostic[]> {
		if (fileNameIsLib(fileName)) {
			return [];
		}
	...
	@TypeScriptWorker.clearFiles
	public async getCompilerOptionsDiagnostics(fileName: string): Promise<Diagnostic[]> {
		if (fileNameIsLib(fileName)) {
			return [];
		}
