...
export class FieldParam extends Blockly.Field<string> {
	...
	protected override onMouseDown_(e: PointerEvent): void {
		...
		if (this.sourceBlock_?.workspace instanceof Blockly.WorkspaceSvg) {
			...
			if (gesture) {
				const block = this.sourceBlock_.workspace.newBlock(
					"parameter",
				) as CustomBlock.Infer<typeof ParameterBlock>; // Create a new block.
	...
	protected override updateSize_() {
		...
		if (this.fieldGroup_ && workspace instanceof Blockly.WorkspaceSvg) {
			const block = ParameterBlock.createIn(workspace);
