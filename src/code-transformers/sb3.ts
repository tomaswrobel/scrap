...
class SB3 {
	declare private target: SB3.Target;
	declare private provided: Record<string, string>;
	declare private assetMap: Record<string, string>;
	...
	private operator(type: "compare" | "arithmetics" | "operation", operator: string) {
	...
	private async input([shadow, data]: SB3.Input, command = false) {
		if (typeof data === "string") {
			const block = await this.block(this.target.blocks[data], !command);
	 ...
	 */
	private override({opcode, inputs = {}, extraState}: SB3.Override) {
		return async (data: SB3.Block) => {
	 ...
	 */
	private asset(type: string) {
		return (data: SB3.Block) => {
			if (data.fields.NUMBER_NAME[0] === "number") {
